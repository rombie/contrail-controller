...
TEST_P(BgpPeerCloseGrTestParam, TestSetGRCapabilities) {
    ...
    for (size_t i = 0; i < capabilities.size(); i++) {
        ...
        if (expected && bgp_peer_close_test_->IsInLlgrTimerWaitState()) {
            ...
            std::vector<uint8_t> afi_flags;
            if (llgr_info)
                afi_flags = llgr_info->afi_flags;
            if (llgr_info &&
                    llgr_info->afi_flags.size() < llgr_info->families.size()) {
                for (size_t i = 0; i < llgr_info->families.size() -
                                       llgr_info->afi_flags.size(); i++) {
                    afi_flags.push_back(0);
                }
            }

            if (expected && llgr_info) {
                size_t i = 0;
                BOOST_FOREACH(uint8_t afi_flag, afi_flags) {
