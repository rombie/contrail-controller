...
func (self *ControlNode) Start() int {
    c1 :=
    "/build/anantha/bgpaas/build/debug/bgp/test/bgp_ifmap_xmpp_integration_test"
...
"CAT_BGP_PORT": strconv.Itoa(self.BgpPort),
"CAT_XMPP_PORT": strconv.Itoa(self.XmppPort),
"BGP_IFMAP_XMPP_INTEGRATION_TEST_INTROSPECT": strconv.Itoa(self.HttpPort),
...
func (self *ControlNode) RestartControlNode() {
...
func (self *Agent) CreateAgent(objs []*ControlNode) {
    var ports []int
    for _, i := range objs {
        fmt.Println(i.Ports)
...
func (self *Agent) Start() int {
    ...
    fmt.Println("Started agent %d", cmd.Process.Pid)
    return cmd.Process.Pid
}

func (self *Agent) CreateConfiguration() {
    sample_conf := "controller/src/bgp/test/cat/lib/contrail-vrouter-agent.conf"
