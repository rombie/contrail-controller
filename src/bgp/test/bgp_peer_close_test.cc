...
// Bring down all agents and stale routes. Agents do not come back up,
// there by causing stale-timer to delete all routes
TEST_P(BgpPeerCloseTest, ClosePeersWithRouteStalingAndDelete_1) {
    SCOPED_TRACE(__FUNCTION__);
    InitParams();
    AddPeersWithRoutes(master_cfg_.get());
    WaitForIdle();
    ...
    VerifyRoutes(n_routes_);
    VerifyRibOutCreationCompletion();
    VerifyReceivedXmppRoutes(
            n_peers_ * n_routes_ + n_instances_ * n_agents_ * n_routes_);

    SetPeerCloseGraceful(true);

    // Trigger ribin deletes
    int down_peers = peers_.size();
    int tmp_down_peers = down_peers;
    BOOST_FOREACH(BgpNullPeer *npeer, peers_) {
        npeer->peer()->Close();
        if (!--tmp_down_peers)
            break;
    }
    tmp_down_peers = down_peers;
    BOOST_FOREACH(BgpNullPeer *npeer, peers_) {
        TASK_UTIL_EXPECT_EQ(tmp_down_peers < 1, npeer->peer()->IsReady());
        tmp_down_peers--;
    }

    int down_agents = xmpp_agents_.size();
    XmppPeerClose(down_agents);
    VerifyRoutes(n_routes_);
    VerifyReceivedXmppRoutes(
            n_peers_ * n_routes_ + n_instances_ * n_agents_ * n_routes_);

    CallStaleTimer(false);
    VerifyReceivedXmppRoutes((n_peers_ - down_peers) * n_routes_ +
        n_instances_ * (n_agents_ - down_agents) * n_routes_);
...
// Bring down some of agents and stale routes. Down agents do not come back up,
// there by causing stale-timer to down agents' routes
TEST_P(BgpPeerCloseTest, ClosePeersWithRouteStalingAndDelete_2) {
    SCOPED_TRACE(__FUNCTION__);
    InitParams();
    AddPeersWithRoutes(master_cfg_.get());
    WaitForIdle();
    ...
    // Trigger ribin deletes
    int down_peers = peers_.size() > 1 ? peers_.size()/2 : peers_.size();
    int tmp_down_peers = down_peers;
    BOOST_FOREACH(BgpNullPeer *npeer, peers_) {
        npeer->peer()->Close();
        if (!--tmp_down_peers)
    ...
    int down_agents = xmpp_agents_.size() > 1 ? xmpp_agents_.size()/2 :
                                               xmpp_agents_.size();
    XmppPeerClose(down_agents);
    VerifyRoutes(n_routes_);
    VerifyReceivedXmppRoutes(
...
// Bring down all agents and stale routes. All agents do come back up,
// there by causing all routes to be retained, before and after agents
// come back up
TEST_P(BgpPeerCloseTest, ClosePeersWithRouteStaling_1) {
    SCOPED_TRACE(__FUNCTION__);
    InitParams();
    ...
    VerifyReceivedXmppRoutes((n_peers_ - down_peers) * n_routes_ +
        n_instances_ * n_agents_ * n_routes_);
...
// Bring down all agents and stale routes. All agents do come back up,
// there by causing all routes to be retained, before and after agents
// come back up. However, agents advertise only half of the routes advertised
// before.
TEST_P(BgpPeerCloseTest, ClosePeersWithRouteStaling_2) {
    SCOPED_TRACE(__FUNCTION__);
    InitParams();
    ...
    WaitForIdle();
    Subscribe();
    int n_routes = n_routes_ > 1 ? n_routes_/2 : n_routes_;
    VerifyReceivedXmppRoutes(
            n_peers_ * n_routes_ + n_instances_ * n_agents_ * n_routes_);
    AddOrDeleteXmppRoutes(true, n_routes, down_agents);
    WaitForIdle();
    VerifyReceivedXmppRoutes(
            n_peers_ * n_routes + n_instances_ * n_agents_ * n_routes_);
    ...
    VerifyReceivedXmppRoutes((n_peers_ - down_peers) * n_routes +
        n_instances_ * (n_agents_ - down_peers) * n_routes_ +
        n_instances_ * n_agents_ * down_peers * n_routes);
...
// Bring down some of agents and stale routes. Those agents do come back up,
// there by causing all routes to be retained, before and after agents
// come back up
TEST_P(BgpPeerCloseTest, ClosePeersWithRouteStaling_3) {
    SCOPED_TRACE(__FUNCTION__);
    InitParams();
    ...
    int n_routes = n_routes_;
    AddOrDeleteXmppRoutes(true, n_routes);
    WaitForIdle();
    VerifyReceivedXmppRoutes(
            n_peers_ * n_routes + n_instances_ * n_agents_ * n_routes_);
    ...
    VerifyReceivedXmppRoutes((n_peers_ - down_peers) * n_routes +
        n_instances_ * n_agents_ * n_routes);
...
// Bring down some of agents and stale routes. Those agents do come back up,
// there by causing all routes to be retained, before and after agents
// come back up. Also, agents advertise only some of the routes advertised
// before (not all)
TEST_P(BgpPeerCloseTest, ClosePeersWithRouteStaling_4) {
    SCOPED_TRACE(__FUNCTION__);
    InitParams();
    ...
    VerifyReceivedXmppRoutes((n_peers_ - down_peers) * n_routes +
        n_instances_ * (n_agents_ - down_agents) * n_routes_ +
        n_instances_ * n_agents_ * down_agents * n_routes);
