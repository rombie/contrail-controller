 ...
 */
#include "bgp/xmpp_rtarget_manager.h"
...
XmppRTargetManager::XmppRTargetManager(
    BgpServer *bgp_server, BgpXmppChannel *bgp_xmpp_channel) :
        bgp_xmpp_channel_(bgp_xmpp_channel), bgp_server_(bgp_server) {
}

bool XmppRTargetManager::IsSubscriptionEmpty() const {
    return bgp_xmpp_channel_->IsSubscriptionEmpty();
}

bool XmppRTargetManager::IsSubscriptionGrStale(
        RoutingInstance *instance) const {
    return bgp_xmpp_channel_->IsSubscriptionGrStale(instance);
}

bool XmppRTargetManager::IsSubscriptionLlgrStale(
        RoutingInstance *instance) const {
    return bgp_xmpp_channel_->IsSubscriptionLlgrStale(instance);
}

bool XmppRTargetManager::delete_in_progress() const {
    return bgp_xmpp_channel_->delete_in_progress();
}

const IPeer *XmppRTargetManager::Peer() const {
    return bgp_xmpp_channel_->Peer();
}

const XmppRTargetManager::RouteTargetList &
XmppRTargetManager::GetSubscribedRTargets(RoutingInstance *instance) const {
    return bgp_xmpp_channel_->GetSubscribedRTargets(instance);
}

uint32_t XmppRTargetManager::GetRTargetRouteFlag(
...
BgpTable *XmppRTargetManager::GetRouteTargetTable() const {
...
void XmppRTargetManager::RTargetRouteOp(as4_t asn, const RouteTarget &rtarget,
    ...
    GetRouteTargetTable()->Enqueue(&req);
}

BgpAttrPtr XmppRTargetManager::GetRouteTargetRouteAttr() const {
    BgpAttrSpec attrs;
...
void XmppRTargetManager::ASNUpdateCallback(as_t old_asn,
    ...
    for (PublishedRTargetRoutes::const_iterator it = rtarget_routes_.begin();
        ...
        RTargetRouteOp(bgp_server_->local_autonomous_system(), it->first, attr,
                       true);
    }
}

void XmppRTargetManager::IdentifierUpdateCallback(
...
void XmppRTargetManager::AddNewRTargetRoute(RoutingInstance *rtinstance,
    ...
    if (rt_loc == rtarget_routes_.end()) {
        ...
        RTargetRouteOp(bgp_server_->local_autonomous_system(), rtarget, attr,
                       true);
    }
    rt_loc->second.insert(rtinstance);
}

void XmppRTargetManager::DeleteRTargetRoute(
    ...
    if (rt_loc->second.empty()) {
        ...
        RTargetRouteOp(bgp_server_->local_autonomous_system(), rtarget, NULL,
                       false);
    }
}

void XmppRTargetManager::RoutingInstanceCallback(RoutingInstance *rt_instance,
...
void XmppRTargetManager::UpdateRouteTargetRouteFlag(
...
void XmppRTargetManager::Close() {
    if (rtarget_routes_.empty())
        return;

    for (PublishedRTargetRoutes::iterator it = rtarget_routes_.begin();
            it != rtarget_routes_.end(); it++) {
        RTargetRouteOp(bgp_server_->local_autonomous_system(), it->first, NULL,
                       false);
    }
    rtarget_routes_.clear();
}

void XmppRTargetManager::Stale(const RouteTargetList &targets) const {
    ...
    BOOST_FOREACH(RouteTarget rtarget, targets) {
        ...
        // Send rtarget route ADD
        RTargetRouteOp(bgp_server_->local_autonomous_system(), rtarget,
                       GetRouteTargetRouteAttr(), true);
    }
}

// Add/Delete rtarget route for import route target of the routing instance.
void XmppRTargetManager::PublishRTargetRoute(RoutingInstance *rt_instance,
                                             bool add_change) {
    if (IsSubscriptionEmpty())
        return;
...
void XmppRTargetManager::FillInfo(BgpNeighborRoutingInstance *instance,
