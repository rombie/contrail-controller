...
BgpRoute *MvpnTable::ReplicateType7Path(MvpnTable *src_table,
...
BgpRoute *MvpnTable::ReplicateType3Path(MvpnTable *src_table,
    MvpnRoute *source_rt, const BgpPath *src_path, ExtCommunityPtr community) {
}
...
// information.
void MvpnTable::ReplicateType4Path(MvpnTable *src_table,
    MvpnRoute *source_rt, const BgpPath *src_path, ExtCommunityPtr community) {
...
BgpRoute *MvpnTable::RouteReplicate(BgpServer *server, BgpTable *src_table,
    ...
    if (source_rt->IsType7()) {
        return ReplicateType7Path(dynamic_cast <MVpnTable *>(src_table),
            dynamic_cast<MvpnRoute *>(source_rt), src_path, community);
    }

    if (source_rt->IsType3()) {
        return ReplicateType3Path(dynamic_cast <MVpnTable *>(src_table),
            dynamic_cast<MvpnRoute *>(source_rt), src_path, community);
    }

    if (source_rt->IsType4()) {
        return ReplicateType4Path(dynamic_cast <MVpnTable *>(src_table),
            dynamic_cast<MvpnRoute *>(source_rt), src_path, community);
    }
}
