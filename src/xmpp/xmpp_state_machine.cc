...
void XmppStateMachine::ProcessStreamHeaderMessage(XmppSession *session,
    ...
    if (!endpoint || !endpoint->connection() ||
        ...
        Enqueue(xmsm::EvXmppOpen(session, msg));
        return;
    }

    // Check if the IP addresses do not match.
    ...
    // If old connection is READY and GR Helper mode is not enabled, ignore
    // the new connection request.
    if (channel->GetPeerState() == xmps::READY &&
            !xmpp_server->IsPeerCloseGraceful()) {
        XMPP_NOTICE(XmppDeleteConnection, session->ToUVEKey(),
                    XMPP_PEER_DIR_IN,
                    "Drop new xmpp connection " + session->ToString() +
                    " as current connection is already up and ready");
    ...
    //
    // Bring down old session if connection is already up and ready (and GR
    // Helper mode is enabled). This is the scenario in which old session's
    // TCP did not learn the session down event, possibly due to compute cold
    // reboot. In that case, trigger closure (and possibly GR) process for
    // the old session.
    if (channel->GetPeerState() == xmps::READY) {
